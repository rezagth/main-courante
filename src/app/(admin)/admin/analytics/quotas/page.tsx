...
type AdminPayload = { quotas: Array<{ tenantId: string; tenantName: string; activeUsers: number; entriesLast30Days: number; storageMb: number }> };
      ...
      <Panel title="Consommation des quotas">
        <div className="space-y-3">
          {(data?.quotas ?? []).map((quota) => {
            const storagePct = Math.min(100, Math.round((quota.storageMb / 1024) * 100));
            return (
              <div key={quota.tenantId} className="rounded-2xl border border-white/5 bg-white/[0.02] p-4">
                ...
                <p className="mt-2 text-xs text-zinc-500">Stockage estimé: {quota.storageMb.toFixed(0)} Mb</p>
              </div>
            );
          })}
        </div>
